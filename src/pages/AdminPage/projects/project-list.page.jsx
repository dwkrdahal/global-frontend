  ...
  return (
    <>
      <PageTitle
        ...
        link={{ to: "/admin/project/create", label: "Create Project", icon: "fas fa-paper-plane" }}
      />

      <Container>
        <Row>
          {(projects && projects.length > 0) ? projects.map((project, index) => (
            <Col key={project._id} xs={12} sm={12} md={6} lg={6} xl={3}  className="mb-4">
              <Card className="project-card h-100">
                ...
                <Card.Body className="d-flex flex-column">
                  ...
                  <div className="project-meta">
                    <span><strong>Location:</strong> {project?.location?.address}</span>
                    <span><strong>Style:</strong> {project?.architectureStyle}</span>
                    <span><strong>Status:</strong> {project?.projectStatus}</span>
                  </div>
                  <NavLink to={project?._id} variant="primary" className=" btn btn-outline-info mt-auto">View Details</NavLink>
                </Card.Body>
              </Card>
            </Col>
          )): <>Data Not found</>}
